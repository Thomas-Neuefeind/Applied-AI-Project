...
def coarse_grid_results(X_train, Y_train, X_test, Y_test):
    ...
    clf = GridSearchCV(estimator=KNeighborsClassifier(), param_grid=grid_params, scoring='accuracy', cv=5)
    # fitting to the clf
...
def main():
    ...
    clf = KNeighborsClassifier(n_neighbors=1, weights="uniform", algorithm="ball_tree", p=1)
